...
|----------|----------|----------|------|------|
| **價格** | 當前價格 | $634.53 | 🟡 | 位於短期均線附近 |
| **均線** | MA20 | $592.34 | 🟢 | 價格在MA20上方 |
| **均線** | MA50 | $631.19 | 🟢 | 價格在MA50上方 |
| **均線** | MA200 | $681.32 | 🔴 | 價格在MA200下方 |
| **動能** | RSI(14) | 57.49 | 🟡 | 中性區間，無超買或超賣 |
| **動能** | MACD | -3.923 | 🟢 | 多頭訊號，柱狀圖正值 |
| **波動** | ATR | $22.84 | 🟡 | 中等波動性 |

### 🏆 技術評分卡

```
技術評分卡 — META (2026-04-14)
════════════════════════════════════════════════════
趨勢強度      ███████░░░░░░░░░░░░░  55/100  🟡
動能指標      ████████░░░░░░░░░░░░  65/100  🟢
支撐穩定度    ██████████░░░░░░░░░░  70/100  🟢
...
```mermaid
graph TD
    ADX["ADX: 21.58"] -->|弱趨勢/盤整| ADX_RESULT["盤整行情"]
```
...
```mermaid
graph TD
    HEAD["頭肩頂形態"] --> BREAKOUT["突破確認"]
    DOUBLE_BOTTOM["雙底形態"] --> TARGET["目標價測算"]
```
...
```mermaid
graph TD
    RSI["RSI: 57.49"] -->|中性| MACD["MACD: -3.923"]
    MACD -->|多頭| BB["布林通道"]
    BB -->|接近上軌| ATR["波動性"]
```
...
```mermaid
graph TD
    MA20["MA20: $592.34"] -->|價格上方| B["看多"]
    MA50["MA50: $631.19"] -->|價格上方| B
    MA200["MA200: $681.32"] -->|價格下方| C["看空"]
```
...
```mermaid
graph TD
    MACD_LINE["MACD Line: -3.923"] --> HISTOGRAM["+9.725"]
    SIGNAL_LINE["Signal Line: -13.648"] --> HISTOGRAM
```
...
|--------|----------|------|----------|------|----------|------|
| 日線 | 上升 | 強 | 空頭 | 完整 | 65 | 持有 |
| 週線 | 盤整 | 中性 | 中性 | 完整 | 60 | 觀望 |
| 月線 | 下降 | 弱 | 空頭 | 不完整 | 50 | 減持 |

### 🔲 綜合訊號矩陣

```mermaid
graph TD
    SHORT_TERM["短期(日線)"] --> SIGNALS["多頭訊號"]
    MID_TERM["中期(週線)"] --> SIGNALS
    LONG_TERM["長期(月線)"] --> SIGNALS["空頭訊號"]
```
...
```mermaid
graph TD
    OPTIMISTIC["樂觀 scenario"] --> BASE["基準 scenario"]
    BASE --> PESSIMISTIC["悲觀 scenario"]
```

### 🛡️ 風險管理重要提醒

- 風險因素：市場波動性、政策變動、競爭加劇
- 倉位管理：保持靈活，根據市況調整倉位比例
