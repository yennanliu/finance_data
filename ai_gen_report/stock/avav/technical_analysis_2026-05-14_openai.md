...
|----------|----------|----------|------|------|
| **價格** | 當前價格 | $160.99 | 🔴 | 價格低於所有主要均線 |
| **均線** | MA20 | $185.73 | 🔴 | 價格在MA下方 |
| **均線** | MA50 | $194.88 | 🔴 | 價格在MA下方 |
| **均線** | MA200 | $267.02 | 🔴 | 價格在MA下方 |
| **動能** | RSI(14) | 25.75 | 🟢 | 超賣狀態 |
| **動能** | MACD | -9.674 | 🔴 | 看空 |
| **波動** | ATR | $10.18 | 🟡 | 中等波動 |
...
| 月份 | 收盤價 | 變動 | 特徵 |
|------|--------|------|------|
| 2025-05 | $178.03 | -15.92% | 中期修正 |
| 2025-06 | $284.95 | +60.0% | 強烈反彈 |
| 2025-10 | $369.91 | +30.0% | 高點形成 |
| 2026-05 | $160.99 | -56.5% | 持續下行 |
...
```mermaid
graph TD
    A["趨勢強度計算"] --> B["ADX: 16.86"]
    B --> C["弱趨勢"]
    B --> D["+DI: 16.20, -DI: 30.41"]
    D --> E["空頭趨勢主導"]
```
...
```mermaid
graph TD
    MACD線["MACD: -9.674"] --> 訊號線["Signal: -7.788"]
    訊號線 --> 柱狀圖["Hist: -1.885"]
    柱狀圖 --> 看空訊號
```
...
- ATR: $10.18
- 建議止損距離：2 x ATR = $20.36

### 📐 Beta 與市場相關性

- Beta 低於 1，與市場相關性中等偏低
...
最優策略組合：
1. 🥇 空頭策略：考量近期反彈壓力，設置嚴格止損
2. 🥈 短線多頭：觀察反彈機會，特別是支撐位附近
3. 🥉 側移策略：等待明確趨勢方向再行動
════════════════════════════════════════════════════════
```
