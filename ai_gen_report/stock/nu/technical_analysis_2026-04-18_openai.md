...
|----------|----------|----------|------|------|
| **價格** | 當前價格 | $15.34 | 🟢 | 位於MA20上方 |
| **均線** | MA20 | $14.52 | 🟢 | 價格在MA上方 |
| **均線** | MA50 | $15.24 | 🟡 | 價格接近平衡點 |
| **均線** | MA200 | $15.37 | 🔴 | 價格在MA下方 |
| **動能** | RSI(14) | 79.59 | 🔴 | 超買 |
| **動能** | MACD | 0.095 | 🟢 | 多頭 |
| **波動** | ATR | $0.51 | 🟡 | 中等波動 |
...
```mermaid
graph TD
    Monthly["月線趨勢: 上升"] --> Weekly["週線趨勢: 盤整"]
...
| 時間 | 漲跌幅 | 說明 |
|------|--------|------|
| 2025-04 至 2026-04 | +25% | 波動加大，上升趨勢明顯 |
...
```mermaid
graph TD
    ADX["ADX(14): 19.66"] --> WeakTrend["弱趨勢/盤整"]
    +DI["+DI: 33.44"] --> Bullish["多頭主導"]
    -DI["-DI: 21.44"] --> Bearish["空頭主導"]
```
...
```mermaid
graph TD
    DoubleTop["雙頂形態"] --> TargetPrice["目標價計算"]
```
...
```mermaid
graph TD
    RSI["RSI(14): 79.59"] --> Overbought["超買"]
    MACD["MACD: 0.095"] --> BullishSignal["多頭訊號"]
    ATR["ATR(14): 0.51"] --> Volatility["中等波動"]
```
...
```mermaid
graph TD
    MA20["短期 MA20: $14.52"] -->|"價格上方"| Bullish["看多"]
    MA50["中期 MA50: $15.24"] -->|"接近平衡"| Neutral["中性"]
    MA200["長期 MA200: $15.37"] -->|"價格下方"| Bearish["看空"]
```
...
```mermaid
graph TD
    MACDLine["MACD線: 0.095"] --> SignalLine["訊號線: -0.108"]
    Histogram["柱狀圖: +0.203"] --> BullishHistogram["看多"]
```
...
|--------|----------|------|----------|------|----------|------|
| 長期 | 上升 | 強 | 看空 | 雙頂 | 60/100 | 持有 |
| 中期 | 盤整 | 中等 | 中性 | N/A | 55/100 | 觀望 |
...
```mermaid
graph TD
    Start["開始"] --> LongStrategy["多頭策略"]
    LongStrategy --> ShortStrategy["空頭策略"]
    ShortStrategy --> End["結束"]
```
...
```mermaid
graph TD
    Optimistic["樂觀情境"] --> Base["基礎情境"] --> Pessimistic["悲觀情境"]
```
