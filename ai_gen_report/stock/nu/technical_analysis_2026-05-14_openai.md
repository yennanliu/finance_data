...
|----------|----------|----------|------|------|
| **價格** | 當前價格 | $12.82 | 🔴 | 價格低於主要均線 |
| **均線** | MA20 | $14.37 | 🔴 | 價格在MA下方 |
| **均線** | MA50 | $14.40 | 🔴 | 價格在MA下方 |
| **均線** | MA200 | $15.47 | 🔴 | 價格在MA下方 |
| **動能** | RSI(14) | 26.70 | 🟢 | 超賣 |
| **動能** | MACD | -0.368 | 🔴 | 看空 |
| **波動** | ATR | $0.40 | 🟡 | 中等波動 |
...
| 月份 | 收盤價 | 漲跌幅 | 特徵 |
|------|--------|--------|------|
| 2026-01 | $17.75 | +10% | 最高點 |
| 2026-05 | $12.82 | -15% | 目前位置 |
...
```mermaid
graph TD
    ADX["ADX指標：19.35"] -->|弱趨勢| VERDICT{"盤整\\n不明顯趨勢"}
```
...
| 月份 | 收盤價 | K線形態 | 解讀 | 訊號 |
|------|--------|---------|------|------|
| 2026-02 | $14.98 | 上影線 | 反轉信號 | 🔴 |
| 2026-05 | $12.82 | 十字星 | 觀望 | 🟡 |
...
```mermaid
graph TD
    MACD["MACD線 -0.368"] -->|低於訊號線| SIGNAL["-0.208"]
    SIGNAL -->|柱狀圖負值| HIST["-0.160"]
    HIST -->|看空| VERDICT{"偏空"}
```
...
當前量能低於平均，成交量未能支持價格的上漲趨勢，顯示出量價背離現象。
...
- ATR 數值：$0.40
- 建議止損距離：1.5x ATR ≈ $0.60

### 📐 Beta 與市場相關性

當前無 Beta 數據，無法進一步分析與大盤相關性。
...
|--------|----------|------|----------|------|----------|------|
| 日線 | 空頭 | 弱 | 空頭 | 不確定 | 40/100 | 觀望 |
| 週線 | 空頭 | 弱 | 空頭 | 不確定 | 45/100 | 觀望 |
| 月線 | 空頭 | 弱 | 空頭 | 不確定 | 50/100 | 觀望 |

### 🔲 綜合訊號矩陣

短期、中期、長期訊號普遍偏空，建議持續觀察。
...
```mermaid
graph TD
    START["開始"] -->|價格低於均線| STRATEGY_A["策略A-觀望"]
    START -->|價格突破阻力| STRATEGY_B["策略B-短線操作"]
```
...
```mermaid
graph TD
    SCENARIO_A["樂觀情景"] --> OUTCOME_A["價格回升至 $16.00"]
    SCENARIO_B["基準情景"] --> OUTCOME_B["價格維持盤整"]
    SCENARIO_C["悲觀情景"] --> OUTCOME_C["價格跌破 $12.00"]
```
