...
|----------|----------|----------|------|------|
| **價格** | 當前價格 | $370.50 | 🟢 | 價格接近52W高 |
| **均線** | MA20 | $349.70 | 🟢 | 價格在MA上方 |
| **均線** | MA50 | $353.74 | 🟢 | 價格在MA上方 |
| **均線** | MA200 | $296.30 | 🟢 | 價格在MA上方 |
| **動能** | RSI(14) | 70.82 | 🔴 | 超買狀態 |
| **動能** | MACD | 7.378 | 🟢 | 多頭動能 |
| **波動** | ATR | $13.05 | 🟡 | 中等波動 |
...
```mermaid
graph TD
    Monthly["月線趨勢"] -->|上升趨勢| Weekly["週線趨勢"]
    Weekly -->|上升趨勢| Daily["日線趨勢"]
    Daily -->|盤整| Final["綜合趨勢：上升"] 
```
...
| 月份 | 收盤價 | 漲跌幅 | 特徵 |
|------|--------|--------|------|
| 2025-07 | 239.54 | +6.67% | 強勢突破 |
| 2025-09 | 277.76 | +20.19% | 加速上攻 |
| 2026-02 | 373.53 | +13.32% | 持續走強 |
...
```mermaid
graph TD
    ADX["ADX 分析"] --> WeakTrend["弱趨勢"]
    WeakTrend --> Conclusion["盤整階段，需等待趨勢突破"]
```
...
```mermaid
graph TD
    HeadShoulder["頭肩頂形態"] --|確認| ConfirmedPattern["確認形態"]
    DoubleBottom["雙底形態"] --|形成中| PotentialPattern["潛在形態"]
```
...
| 月份 | 收盤價 | K線形態 | 解讀 | 訊號 |
|------|--------|---------|------|------|
| 2025-12 | 303.04 | 長陽線 | 上升動能強 | 🟢 |
| 2026-03 | 337.95 | 錘子線 | 反轉可能 | 🟡 |
...
```mermaid
graph TD
    MA["移動平均線"] --> |上升排列| Bullish["看多"]
    RSI["RSI"] --> |超買區間| Bearish["看空"]
    MACD["MACD"] --> |多頭動能| Bullish
```
...
```mermaid
graph TD
    ShortMA["短期 MA20"] -->|價格上方| Bullish
    LongMA["長期 MA200"] -->|價格下方| Bullish
```
...
```mermaid
graph TD
    MACDLine["MACD 線"] -->|高於訊號線| Bullish
    SignalLine["訊號線"] -->|低於 MACD 線| Bullish
    Histogram["柱狀圖"] -->|正值| Bullish
```
...
```mermaid
graph TD
    EntrySignal["進場訊號"] -->|確認| LongStrategy["多頭策略"]
    LongStrategy -->|目標價達成| Exit["退出"]
```
...
最優策略組合：
1. 🥇 最佳機會：中期多頭策略，目標價 $389
2. 🥈 次選機會：短期觀望，等待突破
3. 🥉 保守選擇：長期持有，止損設置於 $350
════════════════════════════════════════════════════════
```
