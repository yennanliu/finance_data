...
|----------|----------|----------|------|------|
| **價格** | 當前價格 | $73.66 | 🟡 | 位於中間區間 |
| **均線** | MA20 | $76.60 | 🔴 | 價格在MA下方 |
| **均線** | MA50 | $85.59 | 🔴 | 價格在MA下方 |
| **均線** | MA200 | $79.61 | 🔴 | 價格在MA下方 |
| **動能** | RSI(14) | 46.05 | 🟡 | 中性 |
| **動能** | MACD | -4.628 | 🟢 | 看多 |
| **波動** | ATR | $5.29 | 🟡 | 中等波動 |
...
| 時間段 | 開始價 | 結束價 | 漲跌幅 | 特徵 |
|--------|--------|--------|--------|------|
| 2025-04 至 2025-09 | $33.78 | $91.37 | +170% | 強勁上漲 |
| 2025-09 至 2026-01 | $91.37 | $103.01 | +12.7% | 高點震盪 |
| 2026-01 至 2026-04 | $103.01 | $73.66 | -28.5% | 下跌修正 |
...
```mermaid
graph TD
    ADX分析["ADX 分析"] --> ADX結果["弱趨勢"]
    ADX分析 --> +DI結果["+DI < -DI"]
    ADX分析 --> -DI結果["空頭主導"]
    ADX結果 --> 總結["盤整狀態"]
```
- **ADX值**：21.20，表示趨勢強度低，市場可能進入盤整狀態。
...
| 月份 | 收盤價 | K線形態 | 解讀 | 訊號 |
|------|--------|---------|------|------|
| 2026-01 | $103.01 | 長上影線 | 強阻力 | 🔴 |
| 2026-03 | $70.51 | 十字星 | 反轉訊號 | 🟢 |
...
```mermaid
graph TD
    短期MA20["短期 MA20"] -->|"價格上方"| 短期看空["短期看空"]
    中期MA50["中期 MA50"] -->|"價格下方"| 中期看空["中期看空"]
    長期MA200["長期 MA200"] -->|"價格下方"| 長期看空["長期看空"]
```
...
- **ATR 值**：$5.29，屬於中等波動範圍
- **建議止損距離**：應設置於 ATR 的 1.5 倍，即約 $7.94

### 📐 Beta 與市場相關性

- **Beta 值**：與大盤相關性中等，風險偏中性
...
|--------|----------|------|----------|------|----------|------|
| 短期 | 上行 | 中性 | 看多 | 完整 | 60/100 | 謹慎看多 |
| 中期 | 盤整 | 中性 | 看空 | 不完整 | 40/100 | 觀望 |
| 長期 | 下行 | 弱 | 看空 | 完整 | 30/100 | 看空 |
...
```mermaid
graph TD
    多頭策略["多頭策略"] --> 進場條件["突破短期阻力"]
    空頭策略["空頭策略"] --> 進場條件["跌破短期支撐"]
    綜合策略["綜合策略"] --> 選擇["選擇最佳策略"]
```
...
最優策略組合：
1. 🥇 最佳機會：短期反彈突破$80，設置合理止損
2. 🥈 次選機會：中期盤整觀望，等待趨勢明確
3. 🥉 保守選擇：長期下行空頭策略，設置合理止損
════════════════════════════════════════════════════════
```
