...
|----------|----------|----------|------|------|
| **價格** | 當前價格 | $232.11 | 🔴 | 價格在主要均線下方 |
| **均線** | MA20 | $243.84 | 🔴 | 價格在MA20下方 |
| **均線** | MA50 | $249.22 | 🔴 | 價格在MA50下方 |
| **均線** | MA200 | $234.59 | 🔴 | 價格在MA200下方 |
| **動能** | RSI(14) | 37.26 | 🟡 | 中性偏弱 |
| **動能** | MACD | -1.500 | 🔴 | 多頭動能不足 |
| **波動** | ATR | $6.80 | 🟡 | 中等波動 |
...
```mermaid
graph TD
    A["雙底形態"] -->|"信心度 60%"| B["頸線突破 $237"]
    C["頭肩頂形態"] -->|"信心度 50%"| D["未確認"]
```
...
- 現價：$232.11 位於 38.2% ($247.02) 和 50% ($237.28) 之間，顯示市場仍在消化過去漲幅，等待新的趨勢確認。
...
graph TD
    MA["移動平均線"] -->|"混合排列"| B["中立"]
    RSI["RSI(14)"] -->|"中性偏弱"| C["偏空"]
    MACD["MACD 負值"] -->|"空頭動能"| D["偏空"]
```
...
```mermaid
graph TD
    MACD["MACD -1.500"] -->|"低於訊號線"| Signal["空頭"]
    Histogram["Hist -1.014"] -->|"持續負值"| Bearish["空頭"]
```
...
- ATR: $6.80，屬於中等波動範圍。
- 建議止損距離：約 $7.00，根據 ATR 波動率設置。

### 📐 Beta 與市場相關性

- Beta: 1.461，表明 AMZN 價格變動性高於市場整體。
...
|--------|----------|------|----------|------|----------|------|
| 日線 | 中性觀望 | 弱 | 混合排列 | 雙底 | 35/100 | 觀望 |
| 週線 | 下行 | 弱 | 混合排列 | 未確認 | 30/100 | 觀望 |
| 月線 | 下行 | 弱 | 混合排列 | 未確認 | 40/100 | 觀望 |
...
```mermaid
graph TD
    A["綜合評分 35/100"] --> B["觀望策略"]
    B --> C["等待突破確認"]
```

### 🧭 策略路由

**當前主策略方向 = 中性觀望（依評分 35/100）**

### 🎯 價格情境階梯（Price Scenarios）

| 觸發條件 | 下一目標 | 後續延伸 | 情境含義 |
|----------|----------|----------|----------|
| 突破 R1 $237 | R2 $248 | R3 $258 | 短線轉強 |
| 站穩現價區間 | — | — | 區間震盪 |
| 跌破 S1 $223 | S2 $215 | S3 $211 | 中期轉弱 |

### 📊 情境機率分佈

| 情境 | 機率 | 主要依據 |
|------|------|----------|
| 繼續上行 / 反彈 | 30% | MACD 信號改善 |
| 區間震盪 | 50% | ADX 弱趨勢 |
| 回落 / 再探底 | 20% | RSI 靠近超賣區 |
