...
3. 視覺化圖表（必須大量使用）：
   - **Mermaid 圖表**：流程/關係/趨勢（graph, pie, mindmap, gantt, quadrantChart）
   ...
   **Mermaid 語法關鍵規則（務必遵守）**：
   ...
   - 正確範例：
     ...
     ST --> ST1["催化劑1"]
     ```
   - quadrantChart 必須包含：title, x-axis, y-axis, quadrant-1~4, 數據點格式 `名稱: [x, y]`
4. 深度要求：
...
使用 Mermaid graph TD 展示短期/中期/長期成長驅動力。**重要：所有節點必須先定義再連接，格式範例：**
...
使用 Mermaid quadrantChart 呈現風險機率 vs 衝擊。**重要：必須使用正確的 quadrantChart 語法，格式範例：**
...
使用 Mermaid graph TD 展示各類投資人適配度（成長型/價值型/股息型/短期交易）及原因。**重要：所有節點必須定義並連接，格式範例：**
...
直接從「# {ticker} 基本面深度分析報告」開始，連續輸出到「## 10. 投資建議」結束，中間不要有任何中斷。
═══════════════════════════════════════════════════════════
"""
...
請根據下方提供的 {ticker} 技術數據，撰寫一份**極其詳盡、專業級**的技術分析報告。

═══════════════════════  嚴格要求  ═══════════════════════
1. 語言：全程使用**繁體中文**
2. 格式：完整 Markdown 格式，報告長度必須超過 800 行
3. 視覺化圖表（必須大量使用，每個章節至少 2-3 個圖表）：
   - **Mermaid 圖表**：使用 graph TD/LR、subgraph、quadrantChart 展示指標關係
   - **ASCII 圖表**：必須繪製詳細的價格走勢圖、支撐阻力位示意圖
   - **Unicode 進度條**：各指標強度使用 ▓░█ 進度條（如：████████░░ 80%）
   - **ASCII 方框圖**：使用 ╔═╗╚═╝├┤ 繪製專業評分卡和儀表板
...
10. 視覺指標：使用 🟢🟡🔴 表示多空訊號強度
11. **詳細程度**：每個小節必須有深入分析，不可簡略帶過
12. **歷史走勢圖**：必須繪製 12 個月的 ASCII 價格走勢圖，標注關鍵高低點
═══════════════════════════════════════════════════════════
...
請按以下架構輸出完整報告，**每個章節都必須詳盡展開**，包含豐富的視覺化圖表與精確數值：

# {ticker} 技術分析報告

> **報告日期**：{today} ｜ **語言**：繁體中文 ｜ **數據來源**：Yahoo Finance ｜ **當前價格**：[從數據中提取]
...
|----------|----------|----------|------|------|
| **價格** | 當前價格 | $XX.XX | 🟢/🟡/🔴 | 說明 |
| **均線** | MA20 | $XX.XX | 🟢/🟡/🔴 | 價格在MA上方/下方 |
| **均線** | MA50 | $XX.XX | 🟢/🟡/🔴 | 說明 |
| **均線** | MA200 | $XX.XX | 🟢/🟡/🔴 | 說明 |
| **動能** | RSI(14) | XX | 🟢/🟡/🔴 | 超買/超賣/中性 |
| **動能** | MACD | XX | 🟢/🟡/🔴 | 多頭/空頭 |
| **波動** | ATR | $XX | 🟢/🟡/🔴 | 高/中/低波動 |
...
*報告生成時間：{today} ｜ 數據來源：Yahoo Finance ｜ 分析框架：多時框技術分析*
"""
...
## 絕對禁止（違反即失敗）
以下行為絕對禁止，違反任何一條都會導致報告失敗：
1. **禁止提及篇幅限制**：絕對不能出現「由於篇幅限制」「篇幅所限」「字數限制」等語句
2. **禁止分部分展示**：絕對不能說「將分部分展示」「分段呈現」「待續」「下篇繼續」
3. **禁止詢問是否繼續**：絕對不能說「如需完整報告請告知」「是否需要繼續」「如需其他章節」
4. **禁止部分報告**：絕對不能只輸出部分章節，必須一次性輸出所有 10 個章節的完整內容
5. **禁止開場白道歉**：絕對不能以「非常感謝您的指引」「感謝您提供數據」等開場
6. **禁止結尾縮減**：絕對不能用「以上是報告的前X個章節」「完整報告將包含」等結尾

你必須直接從「# {ticker} 基本面深度分析報告」標題開始，一氣呵成輸出完整的 10 章報告，直到「## 10. 投資建議」章節完整結束。輸出時不要有任何元評論。"""
