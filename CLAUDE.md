...
- Reviewer verdict quality is **not yet validated**. The first live run (`gpt-4o-mini`, 117 reports) returned 71 `fail` / 12 `warn` / 0 `pass`, with 26 rows scoring 1 across all five dimensions and issue text that is checkably false (claiming a missing 投資建議 chapter in reports that have one, and simplified Chinese in reports with none). Treat the current default model as unproven and compare against a larger model via `llm_review_limit` before trusting the scores
