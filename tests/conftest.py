...
# ── Fake LLM SDK response builders ───────────────────────────────────────────
# Lightweight stand-ins matching the response shapes llm.py reads. Provided as
# factories so each test can dial in text / token counts / finish reasons.
...
class FakeAnthropicResponse:
    """Mimics anthropic ``messages.create`` return (``.content``, ``.usage``)."""

    def __init__(self, text: str, input_tokens: int = 10, output_tokens: int = 999):
...
class FakeOpenAIResponse:
    """Mimics openai ``chat.completions.create`` return."""

    def __init__(self, text: str, prompt_tokens: int = 10, completion_tokens: int = 999):
...
class FakeGeminiResponse:
    """Mimics google.genai ``generate_content`` return."""

    def __init__(self, text: str, finish_reason: str = "STOP",
                 prompt_tokens: int = 10, output_tokens: int = 999):
        self.text = text
        reason = type("Reason", (), {"name": finish_reason})()
        cand = type("Candidate", (), {"finish_reason": reason})()
...
@pytest.fixture
def fake_responses():
    """Namespace of the three fake-response builders."""
    return type("FakeResponses", (), {
        "anthropic": FakeAnthropicResponse,
        "openai": FakeOpenAIResponse,
        "gemini": FakeGeminiResponse,
    })
