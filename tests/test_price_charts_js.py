...
price-charts.js renders the Market Data section's derived charts — drawdown,
rolling volatility, the return-distribution histogram, and the financial series.
The arithmetic behind them is Python (tests/test_price_analytics.py and
tests/test_fundamental_analytics.py), so what this covers is the part pytest
cannot reach: series selection, chart kind, the axis captions and explainer, the
crosshair readout and the degradation paths.
...
def test_price_charts_js_parses():
    ...
    every chart in the Market Data section."""
    proc = subprocess.run(
